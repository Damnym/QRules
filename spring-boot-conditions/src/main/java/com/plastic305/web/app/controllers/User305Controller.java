...
//	private static final String tittleView = "User details" ;
	...
	public String guardar(User305 user305, BindingResult bResult, Model model, RedirectAttributes flash) 
	{
		...
		if (!user305Service.exists(user305.getUsername())) {
			String flashMsg = (user305.getId() != null)? "User \"" + user305.getName() + "\" edit succesfully!!": "User" + user305.getName() + "\" add succesfully!!";
	...
	public String eliminar(@PathVariable(value = "id") Long id, Model model, RedirectAttributes flash) 
	{
		if (id > 0) {
			...
			flash.addFlashAttribute("success", "\"" + user305.getName() + "\" user has been removed from the system successfully");
		} else 
			flash.addFlashAttribute("error", "Cliente no puede <= 0");    // cambiar mensajes
